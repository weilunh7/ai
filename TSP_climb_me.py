...
def distance_map():
    map = [[0 for _ in range(path)] 
				   for _ in range(path)]
    for row in range(path):
        for col in range(path):
            map[row][col] = distance(path[row],path[col])
        return map
...
def pathLength(p): #計算總距離
    ...
    for i in range(1,plen):
        dist += distance(citys[i-1], citys[i])
        #print(dist)
    return dist
